...
def mergeTrainTestUniquify():
    ...
    xTestTrainMerged = np.concatenate((x_train_original, x_test_original),  axis=0)
    ...
    print ("size  of original train: ", len(x_train_original))
    # print ("shape of original train: ", (x_train_original.shape))
    print ('Training features shape:', x_train.shape)
    print ("size of original test : ", len(x_test_original))
    print ("size of merged train test : ", len(xTestTrainMerged)) # now shape is (2270484 ,8,4)
    ### To-do: paste on right with labels. Then, sort, remove duplicates, split again.

    ## I don't know how to properly credit this, but ChatGPT wrote the rest of this procedure
    # Load your matrix into a variable, e.g. `data`
    #xTestTrainMerged = np.load('matrix.npy')

    # Flatten the matrix along the first axis to create a 2D array of shape (n_samples, 32)
    xTestTrainMergedFlattened = xTestTrainMerged.reshape(xTestTrainMerged.shape[0], -1)
    print(" Confirming that new shape is  (n_samples, 32) : ",xTestTrainMerged.shape)
    # Create a dictionary to keep track of the number of times each unique row appears in the original data
    unique_counts = {}
    for row in xTestTrainMergedFlattened:
        row_str = str(row)
        if row_str not in unique_counts:
            unique_counts[row_str] = 0
        unique_counts[row_str] += 1

    # Split the unique rows into train and test sets while ensuring that each unique row appears in either the train or test set, but not both
    #unique_counts.keys() returns the unique 32-element rows. for row_str loops over all the thus-stringified rows.
    # row_str[1:-1] removes the brackets at beginning and end of row_str
    # split turns the resulting string into a list of strings 
    # map turns the list of strings into ints, returning a map
    # list converts the map back into a list
    # np.array turns the list into an array of flattened rows:	
    unique_rows = np.array([list(map(int, row_str[1:-1].split())) for row_str in unique_counts.keys()])
    # create a 1D array  counting from 0 to the number of rows in unique_rows minus 1.
    indices = np.arange(unique_rows.shape[0])
    np.random.shuffle(indices)
    train_rows = []
    test_rows = []
    for i in indices:
        if len(train_rows) < int((2/3) * indices.size):
        #if len(train_rows) < int(0.8 * indices.size):
            train_rows.append(i)
        else:
            test_rows.append(i)
    train_data_flat = unique_rows[train_rows].repeat([unique_counts[str(row)] for row in unique_rows[train_rows]], axis=0)
    test_data_flat = unique_rows[test_rows].repeat([unique_counts[str(row)] for row in unique_rows[test_rows]], axis=0)

    # Reshape the train and test sets back to the original shape of (n_samples, 8, 4)
    train_data = train_data_flat.reshape(train_data_flat.shape[0], 8, 4)
    test_data = test_data_flat.reshape(test_data_flat.shape[0], 8, 4)




mergeTrainTestUniquify() 
