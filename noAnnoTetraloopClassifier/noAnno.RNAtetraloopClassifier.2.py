...
def plot_base_pair_frequencies(tetraloopClass,basePairPosition1, basePairPosition2, basePairStats,singleResidueStats):
    ...
    ax.set_xticks(np.arange(len(residueTypes)))
    ax.set_xticklabels(residueTypes)
    print("check 10 residueTypes = ", residueTypes)         
    print("check 10 ax.get_xticklabels() = ", ax.get_xticklabels())
...
def plot_single_residue_frequencies(tetraloopClass,singleResidueStats):
    ...
    plt.legend(residueTypes) #und 2", "Round 3", "Round 4"])
    plt.title("Frequency of residue type by position")
    plt.savefig("./singleResidueFreq_class"+str(h)+".tiff")
    #plt.show()
    plt.clf() # Close the plt 
